...
class ForwardBatch(ForwardBatchDeepSeekMHAMixin):
    ...
    def compute_spec_mrope_positions(
        ...
        if ragged_layout is None:
            ...
            # Ragged target verify packs all real request rows first and pads the
            # remaining graph-tier tokens at the tail.  Applying a request's
            # (often negative) mRoPE delta to those zero-position padding tokens
            # produces negative RoPE cache indices.  It also cannot use the
            # uniform ``view(batch_size, -1)`` mapping above when verify lengths
            # differ.  Resolve each packed token's request directly from the
            # ragged QO indptr and leave graph padding at the safe position zero.
