...
class ModelRunner(ModelRunnerKVCacheMixin):
    ...
    def _dummy_run(self, batch_size: int, run_ctx=None):
        ...
        if (
            ...
            # For DFLASH decoupled verify mode: warmup with verify_token_num query tokens.
            if (
                self.spec_algorithm.is_dflash()
                and not self.is_draft_worker
                and self.server_args.speculative_dflash_verify_token_num is not None
            ):
                num_tokens_per_bs = self.server_args.speculative_dflash_verify_token_num
            else:
        ...
        def get_spec_info():
            ...
            if self.spec_algorithm.is_eagle() or self.spec_algorithm.is_standalone():
                ...
                # Use verify_token_num for target worker warmup to match graph capture shape.
                _dflash_dummy_token_num = (
                    self.server_args.speculative_dflash_verify_token_num
                    if (
                        not self.is_draft_worker
                        and self.server_args.speculative_dflash_verify_token_num
                        is not None
                    )
                ...
                spec_info = DFlashVerifyInput(
                    draft_token=None,
                    positions=None,
                    draft_token_num=_dflash_dummy_token_num,
                    custom_mask=None,
                    capture_hidden_mode=(
