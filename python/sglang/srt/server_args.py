...
class ServerArgs:
    ...
    def _handle_speculative_decoding(self):
        ...
        if self.speculative_algorithm == "DFLASH":
            ...
            if self.speculative_dflash_verify_token_num is not None:
                ...
                if verify_num <= 0:
                    raise ValueError(
                        "DFLASH --speculative-dflash-verify-token-num must be positive, "
                        f"got {verify_num}."
                    )
                if verify_num > block_size:
                    raise ValueError(
                        "DFLASH --speculative-dflash-verify-token-num must be <= "
                        f"block_size (--speculative-num-draft-tokens). "
                        f"Got verify_token_num={verify_num}, block_size={block_size}."
                    )
                logger.info(
                    "DFLASH decoupled verify mode: draft_tokens=%d, verify_tokens=%d",
                    block_size,
                    verify_num,
                )
    ...
    def add_cli_args(parser: argparse.ArgumentParser):
        ...
        parser.add_argument(
            ...
            type=int,
            help="DFLASH only. Number of draft tokens to send to the target model for verification. "
