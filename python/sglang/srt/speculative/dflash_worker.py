...
class DFlashWorker:
    ...
    def speculative_num_steps(self) -> int:
        """Current effective speculative steps (verify_len - 1).

        Required by ``AdaptiveSpecWorker`` protocol.
        For DFlash the draft block size is fixed; only the *verify* side is truncated.
        """
        return self._effective_verify_len - 1
    ...
    def forward_batch_generation(
        ...
        if self.adaptive_controller is not None:
            self.adaptive_controller.on_verify_complete(
                num_correct_drafts_per_req_cpu, batch_size=batch.batch_size()
            )
