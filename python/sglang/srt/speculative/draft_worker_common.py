...
def build_draft_tp_worker(
    *,
    ...
    disable_dp_attention: bool = False,
) -> DraftWorkerBundle:
    ...
    draft_server_args.override(
        ...
        # DFlash materializes its draft KV cache with the target's DP-local
        # cache locations.  Its draft worker must therefore remain a plain TP
        # worker even when the target uses DP attention.
        enable_dp_attention=(
            False if disable_dp_attention else draft_server_args.enable_dp_attention
        ),
    )
