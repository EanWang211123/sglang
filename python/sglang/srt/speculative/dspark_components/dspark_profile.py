...
def run_adaptive_verify_profile(
    *, worker, tree_cache, max_running_requests: int
    ...
    logger.info(
        "DSpark adaptive verify profiling complete: %d cells, rms=%.3fms, "
        "r2=%.6f, mean_relative_error=%.2f%%, max_relative_error=%.2f%%, "
        "bs_probes=%s, batch_token_probes=%s",
        len(cells),
        fit_stats["rms_ms"],
        fit_stats["r2"],
        statistics.fmean(relative_errors),
        max(relative_errors),
        table.bs_probes,
        table.m_probes,
    )
    logger.info("DSpark adaptive verify SPS table: %s", table.to_json())
