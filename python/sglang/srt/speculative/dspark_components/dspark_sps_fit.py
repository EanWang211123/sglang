...
from sglang.srt.speculative.dspark_components.dspark_sps import SpsAdditiveCostTable


def fit_additive_sps_table(*, cells: list[dict]) -> SpsAdditiveCostTable:
    """Fit the shared ``T(bs, M) = bias + alpha(bs) + theta(M)`` model."""
    if len(cells) < 4:
        raise RuntimeError(
            f"Off-diagonal fit needs at least 4 cells, got {len(cells)}."
        )
    bias, alpha, theta, _rel, _stats = fit_additive_sps_components(cells)
...
def fit_additive_sps_components(cells: list[dict], mbin_w: int = 64):
    """Return fitted components and diagnostics used by both profilers."""
