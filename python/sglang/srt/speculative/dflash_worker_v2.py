...
class DFlashWorkerV2(BaseSpecWorker):
    ...
    def __init__(
        ...
        # The draft has a DP-local KV cache, but it is instantiated and run in
        # the attention-TP group.  This keeps all draft collectives local to a
        # DP replica while its cache locations match the target's rows.
        self._draft_dp_context_enabled = server_args.enable_dp_attention
        draft_ps = replace(ps, pp_rank=0)
        if self._draft_dp_context_enabled:
            # The draft has DP attention disabled, so its logical parallel
            # state must describe the local attention-TP replica rather than
            # the target's global TP/DP topology.
            draft_ps = replace(
                draft_ps,
        ...
        with self._draft_context():
            bundle = build_draft_tp_worker(
                ...
                algo_label="DFLASH",
                disable_dp_attention=self._draft_dp_context_enabled,
            )
        ...
        self.draft_tp_context = (
            draft_tp_context if self._draft_dp_context_enabled else empty_context
        )
