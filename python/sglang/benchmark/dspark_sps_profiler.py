...
try:
    ...
    sys.modules["sglang.srt.speculative.dspark_components.dspark_sps"] = _table_module
    _fit_module = _load_module_by_path(
        "dspark_sps_fit",
        _benchmark_dir.parent / "srt/speculative/dspark_components/dspark_sps_fit.py",
    )
    fit_additive_sps_components = _fit_module.fit_additive_sps_components
    fit_additive_sps_table = _fit_module.fit_additive_sps_table
    DEFAULT_TIMEOUT = 60
    get_tokenizer = None
...
def build_additive_table_from_cells(*, cells: list[dict]) -> SpsAdditiveCostTable:
    return fit_additive_sps_table(cells=cells)


def ols_resid_backfit(cells: list, mbin_w: int = 64):
    return fit_additive_sps_components(cells, mbin_w)
