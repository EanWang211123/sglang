# Branch Relationship Graph

<!-- Generated by scripts/render_branch_graph.py; edit the YAML sources instead. -->

Registry updated: `2026-09-14`

Solid arrows are direct bases or ordered stack composition. Dotted arrows are rebased patches.

## Managed branches

```mermaid
flowchart TD
    base_upstream_main["upstream/main<br/>783af667fb"]
    branch_dspark_compact_ragged_verify["PR #38897 / open<br/>fix/dspark-sps-profiler-verify-all<br/>6c5c125dca"]
    base_upstream_main --> branch_dspark_compact_ragged_verify
    click branch_dspark_compact_ragged_verify "https://github.com/sgl-project/sglang/pull/38897"
    branch_dflash_dp_attn["PR #29506 / open<br/>feat/dflash/dp-attn<br/>33f020fe8d"]
    base_upstream_main --> branch_dflash_dp_attn
    click branch_dflash_dp_attn "https://github.com/sgl-project/sglang/pull/29506"
    branch_dspark_tp_budget_sync["PR #31195 / open<br/>fix/fix-dspark-verlen-confict-tp-ranks<br/>b5f638520a"]
    base_upstream_main --> branch_dspark_tp_budget_sync
    click branch_dspark_tp_budget_sync "https://github.com/sgl-project/sglang/pull/31195"
    branch_dspark_runtime_gamma["PR #31016 / open<br/>fix/fix-dspark-compact-confidence-head-with-vargamma<br/>0e73a13332"]
    base_upstream_main --> branch_dspark_runtime_gamma
    click branch_dspark_runtime_gamma "https://github.com/sgl-project/sglang/pull/31016"
    branch_gptq_marlin_moe_bf16_w2_scales["PR #35955 / open<br/>fix/gptq-marlin-moe-bf16-w2-scales<br/>97df61e7e3"]
    base_upstream_main --> branch_gptq_marlin_moe_bf16_w2_scales
    click branch_gptq_marlin_moe_bf16_w2_scales "https://github.com/sgl-project/sglang/pull/35955"
    branch_replayssm_dflash_dspark["PR #36683 / open<br/>feat/ssm-ring-replay/main<br/>5c07ac58bb"]
    base_upstream_main --> branch_replayssm_dflash_dspark
    click branch_replayssm_dflash_dspark "https://github.com/sgl-project/sglang/pull/36683"
    branch_dspark_adaptive["Maintained feature / active<br/>feat/adaptive_spec_dspark/main<br/>9b5515cd61"]
    click branch_dspark_adaptive "https://github.com/EanWang211123/sglang/tree/feat/adaptive_spec_dspark/main"
    branch_adaptive_dspark_replayssm_gptq_marlin_pack["Maintained feature / active<br/>feat/pack/main<br/>d56eaf7e0e"]
    click branch_adaptive_dspark_replayssm_gptq_marlin_pack "https://github.com/EanWang211123/sglang/tree/feat/pack/main"
    branch_xkernel_slo_aware_prefill_controller["Maintained feature / active<br/>xkernel/slo-aware-prefill-controller<br/>5ac9a8d74e"]
    click branch_xkernel_slo_aware_prefill_controller "https://github.com/EanWang211123/sglang/tree/xkernel/slo-aware-prefill-controller"
    branch_dspark_tp_budget_sync -->|"direct base"| branch_dspark_adaptive
    branch_dspark_runtime_gamma -.->|"rebased patch / 4a7c602a7b"| branch_dspark_adaptive
    branch_dspark_compact_ragged_verify -->|"merged branch / 536fed5ef8"| branch_dspark_adaptive
    branch_dspark_adaptive -->|"direct base"| branch_adaptive_dspark_replayssm_gptq_marlin_pack
    branch_replayssm_dflash_dspark -->|"merged branch / eb57fc87b1"| branch_adaptive_dspark_replayssm_gptq_marlin_pack
    branch_gptq_marlin_moe_bf16_w2_scales -->|"merged branch / 635ca6d816"| branch_adaptive_dspark_replayssm_gptq_marlin_pack
    branch_adaptive_dspark_replayssm_gptq_marlin_pack -->|"direct base"| branch_xkernel_slo_aware_prefill_controller
    classDef base fill:#e8eef7,stroke:#607d9b,color:#17202a
    classDef pr fill:#fff3cd,stroke:#b58900,color:#3d3200
    classDef feature fill:#d9f7e8,stroke:#238636,color:#12351d
    class base_upstream_main base
    class branch_dspark_compact_ragged_verify,branch_dflash_dp_attn,branch_dspark_tp_budget_sync,branch_dspark_runtime_gamma,branch_gptq_marlin_moe_bf16_w2_scales,branch_replayssm_dflash_dspark pr
    class branch_dspark_adaptive,branch_adaptive_dspark_replayssm_gptq_marlin_pack,branch_xkernel_slo_aware_prefill_controller feature
```

## Stack: `adaptive-dspark-replayssm-gptq-marlin-pack`

Source: [`adaptive-dspark-replayssm-gptq-marlin-pack.yaml`](stacks/adaptive-dspark-replayssm-gptq-marlin-pack.yaml)

```mermaid
flowchart LR
    stack_base_adaptive_dspark_replayssm_gptq_marlin_pack["feat/adaptive_spec_dspark/main<br/>9b5515cd61"]
    stack_adaptive_dspark_replayssm_gptq_marlin_pack_1["replayssm-dflash-dspark<br/>merged_branch<br/>eb57fc87b1"]
    stack_base_adaptive_dspark_replayssm_gptq_marlin_pack --> stack_adaptive_dspark_replayssm_gptq_marlin_pack_1
    click stack_adaptive_dspark_replayssm_gptq_marlin_pack_1 "https://github.com/sgl-project/sglang/pull/36683"
    stack_adaptive_dspark_replayssm_gptq_marlin_pack_2["gptq-marlin-moe-bf16-w2-scales<br/>merged_branch<br/>635ca6d816"]
    stack_adaptive_dspark_replayssm_gptq_marlin_pack_1 --> stack_adaptive_dspark_replayssm_gptq_marlin_pack_2
    click stack_adaptive_dspark_replayssm_gptq_marlin_pack_2 "https://github.com/sgl-project/sglang/pull/35955"
    stack_adaptive_dspark_replayssm_gptq_marlin_pack_3["ragged-layout-nonblocking-host-construction<br/>feature_commit<br/>d56eaf7e0e"]
    stack_adaptive_dspark_replayssm_gptq_marlin_pack_2 --> stack_adaptive_dspark_replayssm_gptq_marlin_pack_3
    stack_output_adaptive_dspark_replayssm_gptq_marlin_pack["feat/pack/main<br/>active"]
    stack_adaptive_dspark_replayssm_gptq_marlin_pack_3 --> stack_output_adaptive_dspark_replayssm_gptq_marlin_pack
```

## Stack: `dspark-adaptive`

Source: [`dspark-adaptive.yaml`](stacks/dspark-adaptive.yaml)

```mermaid
flowchart LR
    stack_base_dspark_adaptive["upstream/main<br/>783af667fb"]
    stack_dspark_adaptive_1["dspark-tp-budget-sync<br/>direct_base<br/>b5f638520a"]
    stack_base_dspark_adaptive --> stack_dspark_adaptive_1
    click stack_dspark_adaptive_1 "https://github.com/sgl-project/sglang/pull/31195"
    stack_dspark_adaptive_2["dspark-runtime-gamma<br/>rebased_patch<br/>4a7c602a7b"]
    stack_dspark_adaptive_1 --> stack_dspark_adaptive_2
    click stack_dspark_adaptive_2 "https://github.com/sgl-project/sglang/pull/31016"
    stack_dspark_adaptive_3["dspark-profiling-guard<br/>feature_commit<br/>988846a4e0"]
    stack_dspark_adaptive_2 --> stack_dspark_adaptive_3
    stack_dspark_adaptive_4["dspark-batch-aware-graph-tier<br/>feature_commit<br/>f044291e72"]
    stack_dspark_adaptive_3 --> stack_dspark_adaptive_4
    stack_dspark_adaptive_5["dspark-pre-budget-token-tier<br/>feature_commit<br/>4229b39483"]
    stack_dspark_adaptive_4 --> stack_dspark_adaptive_5
    stack_dspark_adaptive_6["dspark-draft-extend-ragged-guard<br/>feature_commit<br/>1956aed8d8"]
    stack_dspark_adaptive_5 --> stack_dspark_adaptive_6
    stack_dspark_adaptive_7["dspark-runtime-config-adaptation<br/>feature_commit<br/>dd72332235"]
    stack_dspark_adaptive_6 --> stack_dspark_adaptive_7
    stack_dspark_adaptive_8["dspark-online-startup-profile<br/>feature_commit<br/>b62ac30bb6"]
    stack_dspark_adaptive_7 --> stack_dspark_adaptive_8
    stack_dspark_adaptive_9["dspark-dp-padded-graph-tier<br/>feature_commit<br/>781cac8983"]
    stack_dspark_adaptive_8 --> stack_dspark_adaptive_9
    stack_dspark_adaptive_10["dspark-compact-ragged-verify<br/>merged_branch<br/>536fed5ef8"]
    stack_dspark_adaptive_9 --> stack_dspark_adaptive_10
    click stack_dspark_adaptive_10 "https://github.com/sgl-project/sglang/pull/38897"
    stack_dspark_adaptive_11["dspark-profile-config-field-migration<br/>feature_commit<br/>74dc5793c9"]
    stack_dspark_adaptive_10 --> stack_dspark_adaptive_11
    stack_dspark_adaptive_12["dspark-merge-formatting<br/>feature_commit<br/>a09c88a379"]
    stack_dspark_adaptive_11 --> stack_dspark_adaptive_12
    stack_dspark_adaptive_13["ragged-layout-nonblocking-host-construction<br/>feature_commit<br/>9b5515cd61"]
    stack_dspark_adaptive_12 --> stack_dspark_adaptive_13
    stack_output_dspark_adaptive["feat/adaptive_spec_dspark/main<br/>active"]
    stack_dspark_adaptive_13 --> stack_output_dspark_adaptive
```

## Stack: `xkernel-slo-aware-prefill-controller`

Source: [`xkernel-slo-aware-prefill-controller.yaml`](stacks/xkernel-slo-aware-prefill-controller.yaml)

```mermaid
flowchart LR
    stack_base_xkernel_slo_aware_prefill_controller["feat/pack/main<br/>d56eaf7e0e"]
    stack_xkernel_slo_aware_prefill_controller_1["slo-aware-prefill-scheduling<br/>rebased_patch<br/>0d98b9b4a4"]
    stack_base_xkernel_slo_aware_prefill_controller --> stack_xkernel_slo_aware_prefill_controller_1
    stack_xkernel_slo_aware_prefill_controller_2["slo-aware-prefill-tests<br/>rebased_patch<br/>2f08f1cacf"]
    stack_xkernel_slo_aware_prefill_controller_1 --> stack_xkernel_slo_aware_prefill_controller_2
    stack_xkernel_slo_aware_prefill_controller_3["slo-aware-prefill-docs<br/>rebased_patch<br/>d14b43945e"]
    stack_xkernel_slo_aware_prefill_controller_2 --> stack_xkernel_slo_aware_prefill_controller_3
    stack_xkernel_slo_aware_prefill_controller_4["slo-aware-spec-decode-integration<br/>rebased_patch<br/>bbb2be5ec8"]
    stack_xkernel_slo_aware_prefill_controller_3 --> stack_xkernel_slo_aware_prefill_controller_4
    stack_xkernel_slo_aware_prefill_controller_5["slo-aware-glm52-dsa-seed-fix<br/>rebased_patch<br/>f812582793"]
    stack_xkernel_slo_aware_prefill_controller_4 --> stack_xkernel_slo_aware_prefill_controller_5
    stack_xkernel_slo_aware_prefill_controller_6["slo-aware-tp-decision-sync<br/>rebased_patch<br/>8bd264330c"]
    stack_xkernel_slo_aware_prefill_controller_5 --> stack_xkernel_slo_aware_prefill_controller_6
    stack_xkernel_slo_aware_prefill_controller_7["slo-aware-tp-sync-optimization<br/>rebased_patch<br/>bc3bf7e7c4"]
    stack_xkernel_slo_aware_prefill_controller_6 --> stack_xkernel_slo_aware_prefill_controller_7
    stack_xkernel_slo_aware_prefill_controller_8["slo-aware-native-topk-profile-shape<br/>rebased_patch<br/>ed953cf434"]
    stack_xkernel_slo_aware_prefill_controller_7 --> stack_xkernel_slo_aware_prefill_controller_8
    stack_xkernel_slo_aware_prefill_controller_9["slo-aware-pcg-integration<br/>rebased_patch<br/>98fbe17a14"]
    stack_xkernel_slo_aware_prefill_controller_8 --> stack_xkernel_slo_aware_prefill_controller_9
    stack_xkernel_slo_aware_prefill_controller_10["slo-aware-profile-kv-lifecycle<br/>rebased_patch<br/>a221eaf697"]
    stack_xkernel_slo_aware_prefill_controller_9 --> stack_xkernel_slo_aware_prefill_controller_10
    stack_xkernel_slo_aware_prefill_controller_11["slo-aware-rebase-formatting<br/>feature_commit<br/>19f74b11f6"]
    stack_xkernel_slo_aware_prefill_controller_10 --> stack_xkernel_slo_aware_prefill_controller_11
    stack_xkernel_slo_aware_prefill_controller_12["slo-aware-current-kv-lifecycle<br/>feature_commit<br/>31368ab1b4"]
    stack_xkernel_slo_aware_prefill_controller_11 --> stack_xkernel_slo_aware_prefill_controller_12
    stack_xkernel_slo_aware_prefill_controller_13["slo-startup-cache-lifecycle-init<br/>feature_commit<br/>b3631b30a7"]
    stack_xkernel_slo_aware_prefill_controller_12 --> stack_xkernel_slo_aware_prefill_controller_13
    stack_xkernel_slo_aware_prefill_controller_14["slo-resolved-decode-graph-config<br/>feature_commit<br/>d73f0b01bf"]
    stack_xkernel_slo_aware_prefill_controller_13 --> stack_xkernel_slo_aware_prefill_controller_14
    stack_xkernel_slo_aware_prefill_controller_15["slo-dp-padded-graph-tier<br/>feature_commit<br/>22812ded36"]
    stack_xkernel_slo_aware_prefill_controller_14 --> stack_xkernel_slo_aware_prefill_controller_15
    stack_xkernel_slo_aware_prefill_controller_16["slo-feature-pack-refresh<br/>feature_commit<br/>8698a8f0ba"]
    stack_xkernel_slo_aware_prefill_controller_15 --> stack_xkernel_slo_aware_prefill_controller_16
    stack_xkernel_slo_aware_prefill_controller_17["slo-resolved-config-migration<br/>feature_commit<br/>86a382f9bc"]
    stack_xkernel_slo_aware_prefill_controller_16 --> stack_xkernel_slo_aware_prefill_controller_17
    stack_xkernel_slo_aware_prefill_controller_18["slo-feature-pack-final-propagation<br/>feature_commit<br/>1ef9b11aec"]
    stack_xkernel_slo_aware_prefill_controller_17 --> stack_xkernel_slo_aware_prefill_controller_18
    stack_xkernel_slo_aware_prefill_controller_19["ragged-layout-nonblocking-host-construction<br/>feature_commit<br/>5ac9a8d74e"]
    stack_xkernel_slo_aware_prefill_controller_18 --> stack_xkernel_slo_aware_prefill_controller_19
    stack_output_xkernel_slo_aware_prefill_controller["xkernel/slo-aware-prefill-controller<br/>active"]
    stack_xkernel_slo_aware_prefill_controller_19 --> stack_output_xkernel_slo_aware_prefill_controller
```
