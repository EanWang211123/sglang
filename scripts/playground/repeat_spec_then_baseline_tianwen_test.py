#!/usr/bin/env python3
"""
两阶段重复测试：先 DFLASH 投机解码 N 轮（每轮冷启动引擎 -> test.py -> 停服），
再无投机解码 N 轮（同样流程）。

用法（在 sglang 仓库根目录）：
    CUDA_VISIBLE_DEVICES=7 python scripts/playground/repeat_spec_then_baseline_tianwen_test.py

服务端启动与下列命令等价（python -m sglang.launch_server，fa3 + DFLASH 等）：
    见 run_engine() 内拼接的 argv。

环境变量：
    CUDA_VISIBLE_DEVICES              默认 7
    ENGINE_PORT                       默认 9004
    TEST_DIR                          test.py 所在目录（cwd），默认 tianwen-test-v2
    DATASET_PATH                      可选；默认与下方手写 test 命令一致（SpecForge 数据路径）
    NUM_SPEC_ROUNDS                   投机阶段轮数，默认 10
    NUM_BASE_ROUNDS                   非投机阶段轮数，默认 10
    SKIP_NON_SPEC_BENCHMARK=1         若设置则跳过第二阶段（仅跑投机解码部分）
"""
...
ENGINE_PORT = int(os.environ.get("ENGINE_PORT", "9004"))
...
DYNAMIC_VERIFY_CONFIG = (
    ...
    "acc-balance-data/results-tianwen-v2-fa3/cglist_entropy_fa3_top1.json"
)
TOKENIZER_PATH = MODEL_PATH


def _launch_server_argv(*, speculative: bool) -> list[str]:
    """与手写 `python3 -m sglang.launch_server ...` 一致（baseline 去掉投机相关项）。"""
    argv = [
        sys.executable,
        "-m",
...
def run_engine(*, speculative: bool):
    """Start server via `python -m sglang.launch_server` and wait for /health_generate."""
    env = os.environ.copy()
    env["SGLANG_DISABLE_CUDNN_CHECK"] = "1"
    env["CUDA_VISIBLE_DEVICES"] = CUDA_VISIBLE_DEVICES

    cmd = _launch_server_argv(speculative=speculative)
    label = "DFLASH speculative" if speculative else "baseline (no speculative)"
...
def _run_phase(name: str, num_rounds: int, *, speculative: bool) -> None:
    for i in range(1, num_rounds + 1):
        ...
        proc = run_engine(speculative=speculative)
        try:
            if not run_test():
...
def main():
    ...
    _run_phase("speculative (DFLASH)", NUM_SPEC_ROUNDS, speculative=True)

    if SKIP_NON_SPEC:
        print("")
        print("[repeat] SKIP_NON_SPEC_BENCHMARK set — skipping baseline phase.")
    else:
        _run_phase("baseline (no speculative)", NUM_BASE_ROUNDS, speculative=False)
