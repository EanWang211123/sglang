...
Workloads (prompts tuned for Qwen3.5 MTP/EAGLE draft-target divergence):
- low: open-ended reasoning / multi-constraint generation (early draft mismatch)
- high: zero-preamble single-token repetition (maximal greedy agreement)
...
LOW_PROMPTS = [
    (
        "Solve this completely with every intermediate step shown: "
        "Find the smallest positive integer n such that n^2 + 17n + 29 is divisible by 113. "
        "Try candidates systematically, show each divisibility check, and give the final answer."
    ),
    (
        "Write a Python program that simulates 30 full turns of a 6-player werewolf game "
        "with hidden roles, secret night actions, public votes, and eliminations. "
        "Include unique dialogue for every player in every phase."
    ),
    (
        "Draft a treaty between two fictional island nations over shared fishing waters. "
        "Write 12 numbered clauses. Each clause must cite a different legal doctrine, "
        "a distinct penalty, and a specific calendar date."
    ),
    (
        "Generate 80 distinct startup pitches. Each pitch must introduce a unique invented acronym, "
        "a different industry, a fictional founder name, and end with a different rhetorical question."
    ),
]
